...
$(document).ready(function () {
    ...
    var id = random_string + "-" + random_number;
...
function pullDataComments() { //Pool to load messages every 2 sec
    getComments(id);
    ...
    $.get(url_base + "/getComments", {}, function (data)
    {
        if (data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                var str = "";
                if (last_update.id == 0) {
                    str += setDivComment(data[i].text_comment, data[i].firstname, data[i].lastname, data[i].created_at);
                ...
                if (str != "") {
                    ...
                    count = count + 1;
                    document.getElementById('count-comments').innerHTML = count;
                }
            }
        }
    });
...
function setDivComment(text_comment, firstname, lastname, date) {
    var text = "";
    var fullname = firstname + " " + lastname;
    text += "<li class='comment'>";
    text += "<div class='own-comment common-border'>";
    text += "<div class='col-lg-2 post-author-left no-padding'>";
    text += "<img class='shadow' src='images/comments/comment-1.jpg' alt=''>";
    text += "</div>";
